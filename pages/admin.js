...
export default function Admin() {
  ...
  if (checking) return <div style={{background:s.bg,minHeight:'100vh'}}></div>
  ...
  const activeMembers = members.filter(m=>m.status==='active'&&m.username!==ADMIN_USERNAME)
  ...
  const MemberRow = ({m}) => (
    <div style={{background:s.bg2,display:'grid',gridTemplateColumns:'1fr 1fr 1fr auto',gap:16,alignItems:'center',padding:'16px 20px',borderBottom:`1px solid ${s.border}`}}>
      ...
      <div>
        ...
        <div style={{fontSize:11,color:s.muted}}>Joined: {new Date(m.trial_start).toLocaleDateString('en-AU')}</div>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 20px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:32}}>
          {[['Total members',stats.total,s.mid],['Pending approval',stats.pending,s.gold],['Active members',stats.active,s.green],['Total listings',stats.listings,s.gold]].map(([label,val,color])=>(
            <div key={label} style={{background:s.bg3,border:`1px solid ${s.border}`,padding:24,borderRadius:2}}>
        ...
        <div style={{background:s.bg2,border:`1px solid ${s.border}`}}>
          {loading ? (
            ...
            pendingMembers.length===0 ?
              <div style={{padding:40,textAlign:'center',color:s.muted}}>🎉 No pending members — all caught up!</div> :
