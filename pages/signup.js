  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        router.push(`/welcome?name=${encodeURIComponent(form.firstName)}&email=${encodeURIComponent(form.email)}`)
      }
    } catch {
      ...
      <div className="auth-grid">
        <div className="auth-left" style={{background:s.bg3,borderRight:`1px solid ${s.border}`,padding:'48px 40px',flexDirection:'column',justifyContent:'space-between'}}>
          <Link href="/" style={{textDecoration:'none'}}>
            <img src="/Offmarketproplogo5.png" alt="Off Market Property Network" style={{height:44,width:'auto',objectFit:'contain'}}/>
          </Link>
          <div>
            ...
            <p style={{fontSize:14,color:s.muted,lineHeight:1.7}}>No credit card required. No obligation. Join now while we grow the network.</p>
          </div>
          ...
          <form style={{display:'flex',flexDirection:'column',gap:14,maxWidth:480}} onSubmit={handleSubmit}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                ...
                <input name="firstName" value={form.firstName} onChange={handleChange} type="text" placeholder="Jane" style={input} required/>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                <label style={lab}>Last name *</label>
                <input name="lastName" value={form.lastName} onChange={handleChange} type="text" placeholder="Smith" style={input} required/>
              </div>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              <label style={lab}>Email address *</label>
              <input name="email" value={form.email} onChange={handleChange} type="email" placeholder="jane@agency.com.au" style={input} required/>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              <label style={lab}>Mobile number *</label>
              <input name="mobile" value={form.mobile} onChange={handleChange} type="tel" placeholder="04XX XXX XXX" style={input} required/>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              <label style={lab}>Agency name *</label>
              <input name="agency" value={form.agency} onChange={handleChange} type="text" placeholder="Smith Property Group" style={input} required/>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              <label style={lab}>Membership plan</label>
              <select name="plan" value={form.plan} onChange={handleChange} style={{...input,padding:'12px 14px'}}>
                <option value="Bronze">Bronze — $49/mo after trial</option>
                <option value="Silver">Silver — $99/mo after trial (Most popular)</option>
                <option value="Gold">Gold — $179/mo after trial</option>
                <option value="Platinum">Platinum — $349/mo after trial</option>
              </select>
            </div>
            <div style={{borderTop:`1px solid ${s.border}`,paddingTop:14}}>
              <div style={{fontSize:10,letterSpacing:'0.3em',color:s.gold,textTransform:'uppercase',marginBottom:14}}>Create your login</div>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
