...
export default function HowItWorks() {
  const s={gold:'#C9A84C',black:'#0A0A0A',black3:'#1A1A1A',white:'#F5F3EE',muted:'#7A7A7A',mid:'#AAAAAA',border:'#2A2A2A'}
  return (
    <div style={{background:s.black,color:s.white,minHeight:'100vh'}}>
      <Nav/>
      <style>{`
        .step-row{display:grid;grid-template-columns:60px 1fr 1fr;gap:32px;align-items:start;padding:40px 0;border-top:1px solid #2A2A2A;}
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'60px 20px 40px'}}>
        ...
        {steps.map(step=>(
          <div key={step.num} className="step-row">
            ...
            <div style={{background:s.black3,border:`1px solid ${s.border}`,padding:20,display:'flex',flexDirection:'column',gap:12}}>
              {step.details.map(d=>(
      ...
      <section style={{padding:'48px 0',borderTop:`1px solid ${s.border}`,textAlign:'center'}}>
        <div style={{maxWidth:600,margin:'0 auto',padding:'0 20px'}}>
          ...
          <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
            ...
            <Link href="/pricing" style={{border:'1px solid #A8AEBB',color:'#D4D8DF',padding:'14px 28px',fontSize:14,textDecoration:'none'}}>View Pricing</Link>
          </div>
        </div>
      </section>
      <Footer/>
