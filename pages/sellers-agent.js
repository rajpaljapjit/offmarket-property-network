  ...
  return (
    <div style={{ background: c.bg, color: c.white, minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', minHeight: '95vh', display: 'flex', alignItems: 'center', overflow: 'hidden', padding: '60px 0' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(9,9,15,0.92) 0%, rgba(9,9,15,0.72) 55%, rgba(9,9,15,0.88) 100%)' }} />
        <div style={{ position: 'absolute', top: '-10%', right: '-5%', width: '55%', height: '70%', background: 'radial-gradient(ellipse, rgba(184,146,58,0.1) 0%, transparent 65%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: '0', left: '0', width: '45%', height: '55%', background: 'radial-gradient(ellipse, rgba(184,146,58,0.14) 0%, transparent 65%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(rgba(184,146,58,0.05) 1px, transparent 1px)', backgroundSize: '30px 30px', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1200, margin: '0 auto', padding: '0 24px', width: '100%' }}>
          ...
          <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.05 }}
            style={{ fontSize: 'clamp(36px, 5.5vw, 64px)', fontWeight: 700, letterSpacing: '-0.03em', lineHeight: 1.05, marginBottom: 24, maxWidth: 780, color: '#FFFFFF' }}>
            Sell your listings quietly<br />before they{' '}
            <span style={{ background: `linear-gradient(130deg, ${c.gold} 0%, ${c.gold} 50%, #6B9E82 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              hit the market
            </span>
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.15 }}
            style={{ fontSize: 18, color: 'rgba(255,255,255,0.85)', lineHeight: 1.75, marginBottom: 40, maxWidth: 520 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.25 }}
            ...
            <Link href="/signup" style={{ background: c.gold, color: '#1C1A17', padding: '15px 32px', fontSize: 15, fontWeight: 700, textDecoration: 'none', borderRadius: 8, display: 'inline-block' }}>
              Sign Up
            </Link>
            <Link href="/" style={{ border: '1px solid rgba(255,255,255,0.3)', color: '#FFFFFF', padding: '15px 28px', fontSize: 15, textDecoration: 'none', borderRadius: 8, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(6px)', display: 'inline-block' }}>
              Home
            </Link>
          </motion.div>
          ...
          {benefits.map((b, i) => (
            <FadeUp key={b.title} delay={i * 0.07}>
              <div style={{ background: c.bg2, border: `1px solid ${c.border}`, borderRadius: 12, padding: '28px 24px', height: '100%', transition: 'border-color 0.2s, box-shadow 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(184,146,58,0.45)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(184,146,58,0.1)' }}
            ...
            {steps.map((step, i) => (
              <FadeUp key={step.num} delay={i * 0.08}>
                <div style={{ textAlign: 'center', padding: '0 12px' }}>
                  <div style={{ fontSize: 48, fontWeight: 700, color: c.gold, letterSpacing: '-0.04em', marginBottom: 16, textShadow: '0 0 24px rgba(184,146,58,0.3)' }}>{step.num}</div>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: '96px 24px', textAlign: 'center' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto' }}>
          <FadeUp>
            ...
            <h2 style={{ fontSize: 'clamp(28px,4vw,44px)', fontWeight: 700, letterSpacing: '-0.02em', color: '#FFFFFF', marginBottom: 16 }}>
              Start selling off market today
            </h2>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.82)', lineHeight: 1.7, marginBottom: 32 }}>
              Join verified selling agents already sharing listings that never make it to the public portals.
            </p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link href="/signup" style={{ background: c.gold, color: '#1C1A17', padding: '15px 36px', fontSize: 15, fontWeight: 700, textDecoration: 'none', borderRadius: 8, display: 'inline-block' }}>
                Sign Up
              </Link>
              <Link href="/" style={{ border: '1px solid rgba(255,255,255,0.3)', color: '#FFFFFF', padding: '15px 28px', fontSize: 15, textDecoration: 'none', borderRadius: 8, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(6px)', display: 'inline-block' }}>
                Home
