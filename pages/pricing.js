...
export default function Pricing() {
  return (
    <div style={{background:s.bg,color:s.white,minHeight:'100vh'}}>
      <Nav/>
      <style>{`
        .plans-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:#1E2A45;}
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'60px 20px 40px'}}>
        ...
        <div className="plans-grid">
          {plans.map(p=>(
            <div key={p.name} style={{background:p.featured?s.bg3:s.bg2,padding:p.featured?'44px 24px 28px':'28px 24px',position:'relative',display:'flex',flexDirection:'column'}}>
